...
{
    public partial class viewEquipmentscs : Form
    {
        public viewEquipmentscs()
        {
            InitializeComponent();
        }
        ...
        private void dataGridView1_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (dataGridView1.Columns[e.ColumnIndex].Name == "Edit")
            {
                ...
                string id = row.Cells["e_id"].Value.ToString();
                string equipment = row.Cells["equipment"].Value.ToString();
                string quantity = row.Cells["quantity"].Value.ToString();
                string price = row.Cells["price"].Value.ToString();
                string muscle = row.Cells["muscle"].Value.ToString();


                var editForm = new editEquipments(
                    id, equipment, muscle, price, quantity
                );
            ...
            {
                if (dataGridView1.Rows[e.RowIndex].Cells["e_id"].Value != null)
                {
                    try
                    {
                        int equipId = Convert.ToInt32(dataGridView1.Rows[e.RowIndex].Cells["e_id"].Value);
        ...
        {
            string connectionString = "data source=DESKTOP-O2789LR; database=gym; integrated security=true";
            string query = "DELETE FROM equipments WHERE e_id = @equipID";
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                ...
                try
                {
                    ...
                    {
                        MessageBox.Show("No Equipment found with the specified ID.");
                    }
                }
